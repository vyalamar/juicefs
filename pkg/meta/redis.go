...
func (m *redisMeta) doBatchClone(ctx Context, srcParent Ino, dstParent Ino, entries []*Entry, cmode uint8, cumask uint16, length *int64, space *int64, inodes *int64, userGroupQuotas *[]userGroupQuotaDelta) syscall.Errno {
	...
	if userGroupQuotas != nil {
		*userGroupQuotas = (*userGroupQuotas)[:0]
	}
	...
	for start := 0; start < len(entries); start += batchSize {
		...
		err := m.txn(ctx, func(tx *redis.Tx) error {
			...
			batchLength, batchSpace, batchInodes = 0, 0, 0
			if userGroupQuotas != nil {
				batchQuotas = make([]userGroupQuotaDelta, 0, len(infos))
			}
			...
			for _, info := range infos {
				...
				batchLength += int64(sd.attr.Length)
				es := align4K(sd.attr.Length)
				batchSpace += es
				batchInodes++
				if userGroupQuotas != nil {
					batchQuotas = append(batchQuotas, userGroupQuotaDelta{
			...
			_, err = tx.TxPipelined(ctx, func(p redis.Pipeliner) error {
				...
				if batchSpace != 0 {
					p.IncrBy(ctx, m.usedSpaceKey(), batchSpace)
				}
				if batchInodes != 0 {
					p.IncrBy(ctx, m.totalInodesKey(), batchInodes)
				}
		...
		if userGroupQuotas != nil && len(batchQuotas) > 0 {
			*userGroupQuotas = append(*userGroupQuotas, batchQuotas...)
		}
	}
	return 0
}
